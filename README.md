...
- **Embedding model recall gap for specific subsections**: retrieval ranking can miss specific technical subsections (e.g., a paper's dedicated "Fixed-Point Quantization" or "Methodology" section) even when they are directly relevant to a query. Confirmed via direct investigation: a real, correctly-chunked section can rank as low as #23–24 out of ~50 nearest neighbors for a natural-language question specifically about that section's topic, placing it outside `RETRIEVAL_TOP_K=6`. The chunk exists, is clean, and is correctly labeled — it simply does not embed as similar to certain natural-language phrasings of its own topic as other, less-relevant sections do. This appears to be a limitation of the local embedding model (`BAAI/bge-small-en-v1.5`) rather than a chunking or retrieval-logic bug. Candidate future improvements: raising `RETRIEVAL_TOP_K`, evaluating a larger or different embedding model, or adding hybrid keyword+vector search using the existing (currently unused) `tsv`/GIN full-text index already present in `db/schema.sql`.
